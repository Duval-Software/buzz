...
Community owners/admins can publish announcements even when their channel role is `member`. The relay reads their current tenant-scoped role for every publish; the UI uses the verified community roster. Ordinary members and moderators remain read-only.
