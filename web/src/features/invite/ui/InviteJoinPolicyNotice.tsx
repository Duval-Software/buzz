...
function PolicyCheckbox({
  ...
  return (
    <div className="relative">
      ...
      />
      <label
        className="flex cursor-pointer items-start gap-3 rounded-sm text-left text-xs leading-5 text-neutral-300 peer-focus-visible:outline-2 peer-focus-visible:outline-offset-2 peer-focus-visible:outline-amber-400"
        htmlFor={id}
      >
        <span
          aria-hidden="true"
...
export function InviteJoinPolicyNotice({
  ...
  return (
    <div
      className="w-full space-y-3 rounded-xl border border-neutral-700 bg-neutral-900 p-4 text-left"
      ...
      {policy.terms_markdown || policy.privacy_markdown ? (
        ...
        >
          ...
          {policy.terms_markdown ? (
            <button
              className="text-amber-300 no-underline underline-offset-4 hover:text-amber-200 hover:underline focus-visible:outline-1 focus-visible:outline-offset-2 focus-visible:outline-amber-400"
              type="button"
              onClick={(event) =>
                stopLabelActivation(
                  event,
