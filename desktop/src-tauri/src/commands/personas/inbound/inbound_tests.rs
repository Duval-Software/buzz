...
/// Persona-inherited record: runtime=None (cleared by "inherit" update), stale
/// agent_command="openclaw", live persona runtime=goose. Inbound 10 must NOT be
/// capped because the effective command follows the live persona (goose).
///
/// This is the correctness-critical case from Thufir pass-2: without passing
/// personas into apply_inbound_managed_agent, the context-free fallback would
/// read the stale agent_command ("openclaw") and incorrectly cap to 5.
#[test]
fn inbound_persona_inherited_runtime_none_stale_openclaw_live_goose_is_uncapped() {
    use crate::managed_agents::{agent_events::managed_agent_content_from_event, AgentDefinition};
    use nostr::{EventBuilder, JsonUtil, Keys, Kind, Tag};
    ...
    let goose_persona = AgentDefinition {
...
fn inbound_persona_inherited_runtime_none_stale_goose_live_openclaw_is_capped() {
    ...
    let openclaw_persona = AgentDefinition {
